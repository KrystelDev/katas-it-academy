...
let elegit = "";
function pregunta(elegit) {
  console.log("T'agrada " + elegit + "?");
}
pregunta("la pizza");
...
function pregunta2(elegit) {
  return "T'agrada " + elegit;
}
...
function mostrarSolució(m3) {
  let litres = metresCubicsALitres(m3);
...
function afegirDiners(quantitat) {
  diners = diners + quantitat;
}
function gastarDiners(quantitat) {
  diners = diners - quantitat;
}
...
var talla = prompt("Quina talla de samarreta que vols comprar (S, M, L, XL)?"); // TODO: editar el missatge del prompt()

// TODO: estructura condicional que digui per la consola la talla escollida
console.log(talla);
if (talla === "S" || talla === "M" || talla === "L" || talla === "XL") {
  console.log("Has seleccionat: " + talla);
} else {
  console.log("La talla no és vàlida. Selecciona S, M, L o XL");
}
...
var text =
  "En un lugar de La Mancha de cuyo nombre no quiero acordarme. Què tal, Manel?   Patata";
